...
export default function SignUp() {
	const [userType, setUserType] = useState<"applicant" | "employer" | null>(
		null,
	);
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center p-4 md:p-10 bg-gradient-to-b from-gray-50 to-white">
			<div className="w-full max-w-3xl space-y-8 text-center">
				...
				<p>
					Your gateway to career opportunities and top talent. Choose your path
					to get started.
				</p>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
					<Card className="flex flex-col justify-between bg-white hover:shadow-lg transition-all duration-250">
							...
							<ul className="space-y-2 text-sm text-left">
								{jobSeeker.map((text, index) => (
									<li key={index} className="flex items-center">
										<span className="bg-[#ffeedd] flex items-center justify-center text-mainColor rounded-full p-1 w-7 h-7 mr-3">
											✓
										</span>
										{text}
									</li>
						...
						<CardFooter>
							<Button
								onClick={() => handleSignUpButton("applicant")}
								className="w-full bg-mainColor hover:bg-main/90"
							>
					...
					<Card className="flex flex-col justify-between bg-white hover:shadow-lg transition-all duration-250">
						...
						<CardContent>
							<ul className="space-y-2 text-sm text-left">
								{jobSeeker.map((text, index) => (
									<li key={index} className="flex items-center">
										<span className="bg-[#d1d0e0] flex items-center justify-center text-secondaryColor rounded-full p-1 w-7 h-7 mr-3">
											✓
										</span>
										{text}
									</li>
						...
						<CardFooter>
							<Button
								onClick={() => handleSignUpButton("employer")}
								className="w-full bg-secondaryColor hover:bg-main/90"
							>
								Sign Up as Employer
							</Button>
						</CardFooter>
